...
"""Macro Event V3 R2 scorer.

The model layer is deliberately source-agnostic: it reads only governed Neon
observations.  It makes no HTTP/provider calls.  Historical Employment is pinned
to the already-frozen 756-row Macro Event source run.  Historical Inflation is
pinned to the successful V3 R2 existing-channel backfill.  FOMC remains fail-
closed until the contracted market-implied target/path source is configured.
"""
...
def robust_scale(xs: list[float]) -> tuple[float | None, str | None]:
...
def state_from_score(score: float, oriented: list[float], breadth_required: int) -> str:
    adverse = sum(x < 0 for x in oriented); supportive = sum(x > 0 for x in oriented)
    if score <= -STRONG_THRESHOLD and adverse >= breadth_required: return "GOLD_ADVERSE_MACRO_SHOCK"
    if score >= STRONG_THRESHOLD and supportive >= breadth_required: return "GOLD_SUPPORTIVE_MACRO_SHOCK"
    return "MACRO_MIXED_OR_SMALL"


def fetch_rows_for_run(conn, run_id: str, ids: dict[str,str]) -> list[dict]:
...
def latest_inflation_source_run(conn) -> str:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute("""select run_id from retrieval_runs
                       where pipeline_version=%s and status='SUCCESS' and observations_written>0
                       order by finished_at desc limit 1""",(INFLATION_SOURCE_PIPELINE,))
        r=cur.fetchone()
    if not r: raise RuntimeError("INFLATION_EXISTING_CHANNEL_SOURCE_RUN_NOT_FOUND")
    return str(r["run_id"])


def build_panel(rows: list[dict], ids: dict[str,str]) -> list[dict]:
...
def score_employment(panel: list[dict]) -> list[dict]:
    ...
    for r in panel:
        e={"nfp":r["nfp_actual"]-r["nfp_consensus"],
           "unemp":r["unemp_actual"]-r["unemp_consensus"],
           "ahe":r["ahe_actual"]-r["ahe_consensus"]}
        scales={k:robust_scale(prior[k]) for k in prior}
        if all(scales[k][0] is not None for k in prior):
            z={k:e[k]/float(scales[k][0]) for k in prior}
            oriented=[-z["nfp"],z["unemp"],-z["ahe"]]; score=sum(oriented)/3.0
            out.append({"observation_ts":r["observation_ts"],"score":score,
                        "state":state_from_score(score,oriented,2),"components":oriented,
                        "surprises":e,"scale_methods":{k:scales[k][1] for k in prior},
                        "scale_values":{k:scales[k][0] for k in prior}})
...
def score_inflation(panel: list[dict]) -> list[dict]:
    ...
    for r in panel:
        ...
        scales={k:robust_scale(prior[k]) for k in prior}
        if all(scales[k][0] is not None for k in prior):
            z={k:e[k]/float(scales[k][0]) for k in prior}; oriented=[-z["cpi"],-z["core"]]
...
def persist_scores(conn, family: str, scores: list[dict], source_run_id: str, run_id: str, retrieved: datetime) -> int:
    ...
    for s in scores:
        ...
        metadata={"engine":ENGINE,"family":family,"state":s["state"],"components":s["components"],
                  "surprises":s["surprises"],"scale_methods":s["scale_methods"],"scale_values":s["scale_values"],
                  "source_run_id":source_run_id,"evidence_class":"HISTORICAL_REPLAY_RECONSTRUCTION",
                  "historical_consensus_pit":"NOT_PROVEN_PROVIDER_UPDATE_TIMESTAMP",
                  "context_only":True,"direction_vote":False,"market_shock_threshold_changed":False,
                  "raw_market_shock_episode_changed":False,"production_authority":False}
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute("""select value,metadata from observations
                           where series_id=%s and observation_ts=%s and lineage_id=%s
                           order by retrieved_at desc limit 1""",(sid,s["observation_ts"],lineage))
            prev=cur.fetchone()
        if prev:
        ...
        with conn.cursor() as cur:
            cur.execute("""insert into observations
                ...
                (run_id,sid,s["observation_ts"],s["score"],"Derived from governed Macro Event raw observations",None,
                 s["observation_ts"],s["observation_ts"],retrieved,retrieved,"event_monthly","robust_z",
                 "ROBUST_EVENT_SURPRISE_SCORE","RESEARCH_CONTEXT_ONLY",lineage,
                 __import__("hashlib").sha256(json.dumps(metadata,sort_keys=True).encode()).hexdigest(),
...
def main() -> int:
    ...
    with psycopg.connect(db_url(),autocommit=False) as conn:
        ...
        inflation_run=latest_inflation_source_run(conn)
        inf_rows=fetch_rows_for_run(conn,inflation_run,INFLATION_IDS)
        inf_panel=build_panel(inf_rows,INFLATION_IDS)
        ...
        result={"engine_id":ENGINE,"status":"PASS_EMPLOYMENT_INFLATION_EXISTING_CHANNELS_FOMC_BLOCKED",
                "employment":{"source_run_id":EMPLOYMENT_FROZEN_RUN_ID,"raw_events":len(emp_panel),
                              "scored_events":len(emp_scores),"latest":emp_scores[-1] if emp_scores else None},
                "inflation":{"source_run_id":inflation_run,"raw_events":len(inf_panel),
                             "scored_events":len(inf_scores),"latest":inf_scores[-1] if inf_scores else None},
                "fomc":{"status":fomc_status},"model_layer_external_http_calls":False,
                "market_shock_threshold_changed":False,"raw_market_shock_episode_changed":False,
                "production_authority":False,"evidence_class":"HISTORICAL_REPLAY_RECONSTRUCTION"}
        with conn.cursor() as cur:
            cur.execute("""insert into retrieval_runs
                ...
                (score_run_id,retrieved,finished,PIPELINE_VERSION,git_sha(),"macro_event_v3_existing_channels_score",
                 "SUCCESS",len(emp_rows)+len(inf_rows),emp_written+inf_written,
                 "V3 model score from governed Neon channels only; no provider calls; no Market Shock mutation",

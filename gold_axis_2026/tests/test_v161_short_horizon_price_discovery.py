...
def test_price_discovery_join_is_strict_previous_date():
    panel = pd.DataFrame({"date": pd.to_datetime(["2025-01-03", "2025-01-06"]), "close": [2600.0, 2610.0]})
    market = pd.DataFrame({
        "source_date": pd.to_datetime(["2025-01-02", "2025-01-03", "2025-01-06"]),
        "close": [2650.0, 2660.0, 2670.0], "gc_ret1": [0.0, 0.01, 0.02],
    })
...
def test_target_maturity_geometry():
    d = pd.DataFrame({"date": pd.date_range("2025-01-01", periods=10, freq="D"), "close": np.arange(100.0, 110.0)})
...
def test_feature_sets_remove_legacy_generic_fx():
    c = v161.load_contract()

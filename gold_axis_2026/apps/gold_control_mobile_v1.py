...
if nav=="⌂ Bugün":
    ...
    st.markdown("<div class='gc-footnote' style='margin-bottom:.45rem'><b>PROSPECTIVE / LIVE CANONICAL SCORECARD</b> · Historical Replay bu kartlara girmez.</div>",unsafe_allow_html=True); st.markdown("<div class='gc-grid4'>"+mini_card("REALIZED TAHMİN",str(realized_count),"Outcome-linked prospective/live")+mini_card("MAPE","—","YETERLİ VERİ YOK")+mini_card("MAE (USD)","—","YETERLİ VERİ YOK")+mini_card("YÖN DOĞRULUĞU","—","Frozen outcome tanımı sonrası")+"</div>",unsafe_allow_html=True)
    with st.container(border=True):

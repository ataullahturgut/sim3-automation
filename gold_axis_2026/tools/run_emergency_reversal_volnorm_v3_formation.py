...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="/tmp/emergency-reversal-volnorm-v3-formation")
    ...
    if any(d.dayofweek >= 5 for d in closes.index):
        raise RuntimeError("FORMATION_WEEKEND_FAIL")
    ...
    summary = {
        ...
        "source": evidence,
        "config": m.config().as_dict() | {
            "selected_hour": m.SELECTED_HOUR,
            "min_year_coverage": m.MIN_YEAR_COVERAGE,
        ...
        "weekend_rows_consumed": int(sum(d.dayofweek >= 5 for d in closes.index)),

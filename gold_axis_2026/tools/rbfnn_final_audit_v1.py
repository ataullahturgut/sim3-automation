...
def run(root):
    models=s4.load(root)
    valid={n:d for n,d in models.items() if len(d['dev']['rows'])==33 and d['dev'].get('scientific_gate','PASS')=='PASS'}
    ensemble=json.loads((root/'GOLD_MONTHLY_RBFNN_STAGE4_RESULT_2026-09-26.json').read_text())
    ...
    refinements=[n for n in valid if n.startswith('ADAPTIVE') or 'TUNED' in n or n=='PSO_TLBO' or n.startswith('MPA_') or n=='MOLS_RBFNN']
    ...
    cross['RBFNN_'+single]=valid[single];cross['RBFNN_STAGE3_'+refine]=valid[refine];cross['RBFNN_ENSEMBLE_'+ens]=em[ens]
    cross['RBFNN_BEST_HYBRID_'+hybrid]=valid[hybrid]
    cross['RBFNN_STAGE3_HYBRID_PSO_TLBO']=valid['PSO_TLBO']
    ...
    out={'status':'COMPLETE','selection':'DEV_ONLY','primary':primary,'single_leader':single,'stage3_leader':refine,
      ...
      'rbfnn_frontier':frontier,'cross_family_frontier':global_frontier,'cross_family_metrics':summary,
      ...
      'limitations':['n=33 DEV and many model comparisons: point-estimate selection, not statistical superiority',
        'historical baseline artifacts lack full empty-cluster gate proof; NOT_PROVEN, retained as canonical anchors without unnecessary reruns',
      ...
      'next_action':'RBFNN family frozen; return to governed monthly roadmap N2 Gaussian Processes; no automatic N2 execution in this task'}
    (root/'GOLD_MONTHLY_RBFNN_FINAL_FREEZE_2026-09-26.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
    lines=['# GOLD MONTHLY FORECAST — RBFNN final family freeze and cross-family audit','','Status: COMPLETE. Selection: DEV 2022-04..2024-12 only (33 months).','',
      f'RBFNN primary: **{primary}**. Direction specialist: **{direction}**. Balanced challenger: **{balanced}** (benchmark role). Best hybrid: **{hybrid}**. Ensemble leader: **{ens}**, {out["ensemble_decision"]}.','',
    ...
    lines+=['','Full monthly forecasts, prequential weight trajectories, all year blocks, worst-month errors and leave-one-component-out diagnostics are in the Stage-4 JSON. No component/pool changed after ensemble evaluation.','',
      '## Reporting only — no acceptance authority','','Historical family external protocols differ from the strict RBFNN 2024-12 tuning freeze. These are reported values, not a controlled cross-family external ranking.','',

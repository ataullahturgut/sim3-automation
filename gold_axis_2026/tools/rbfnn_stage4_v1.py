...
def load(root):
    models=json.loads(gzip.decompress((root/'evidence/rbfnn_stage1/strict_results.json.gz').read_bytes()))
    archive=root/'evidence/rbfnn_stage3/results.json.gz'
    if archive.exists():models.update(json.loads(gzip.decompress(archive.read_bytes())))
    for path in (root/'evidence/rbfnn_stage3').glob('*.json'):
        d=json.loads(path.read_text())
        if 'dev' in d:models[d['method']]=d
    for n,p in [('VANILLA','vanilla'),('REGULARIZED','regularized')]:
        models[n]=json.loads((root/f'evidence/rbfnn_stage0/vw_midas_rbfnn_{p}_v1_result.json').read_text())
    return models
...
def freeze(root,path):
    assert json.loads((root/'GOLD_MONTHLY_RBFNN_STAGE3_CLOSURE_2026-09-26.json').read_text())['status']=='COMPLETE'
    all_models=load(root)
    models={n:d for n,d in all_models.items() if len(d['dev']['rows'])==33 and d['dev'].get('scientific_gate','PASS')=='PASS'}
    ms={n:metrics(d['dev']['rows']) for n,d in models.items()}
    roles={'architecture_anchor':'VANILLA','price_leader':min(ms,key=lambda n:ms[n]['sum_abs_error']),
...
def evaluate(root,pool_path,out_path):
    ...
    out={'pool_freeze_sha256':hashlib.sha256(pool_path.read_bytes()).hexdigest(),'authority':'DEV_ONLY','pools':{},
      ...
      'external_policy':'ensemble weights frozen after DEV; Stage1/3 optimized geometry and tuning frozen 2024-12; canonical Vanilla uses its fixed, non-tuned origin-only k-means procedure',

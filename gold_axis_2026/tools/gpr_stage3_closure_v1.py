...
def run(root,stage):
 ...
 if stage!='3A':
  for n,entry in zip(hybrid,opening):assert models[n]['stage']=='3B' and models[n]['opening_evidence']==entry
 if stage!='3A':assert json.loads((root/'GOLD_MONTHLY_GPR_STAGE3A_CLOSURE_2026-09-26.json').read_text())['status']=='COMPLETE'
 if stage=='3':
  ab=root/'GOLD_MONTHLY_GPR_STAGE3AB_CLOSURE_2026-09-26.json';authority=root/'GOLD_MONTHLY_GPR_STAGE3C_AUTHORITY_2026-09-26.md'
  assert json.loads(ab.read_text())['status']=='COMPLETE'
  d=models['LMC2_RBF_M32'];assert d['stage3ab_closure_sha256']==hashlib.sha256(ab.read_bytes()).hexdigest()
  assert d['authority_document_sha256']==hashlib.sha256(authority.read_bytes()).hexdigest()

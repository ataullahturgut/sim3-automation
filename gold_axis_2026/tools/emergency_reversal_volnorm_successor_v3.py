...
MODEL_ID = "EMERGENCY_REVERSAL_VOLNORM_SUCCESSOR_V3"
SOURCE_ID = "XAU_TWELVE_1H_SELECT_14_00_NY_WEEKDAY_RESEARCH_V3"
SELECTED_HOUR = 14
...
def fetch_twelve_closes(years: Iterable[int], api_key: str | None = None) -> tuple[pd.Series, dict[str, Any]]:
    ...
    for year in years_sorted:
        for start, end in core._half_year_segments(year):
            ...
            for item in payload.get("values") or []:
                ...
                if day in selected:
                    raise RuntimeError(f"DUPLICATE_SELECTED_WEEKDAY_BAR:{text}")
                selected[day] = value
    ...
    if any(pd.Timestamp(d).dayofweek >= 5 for d in series.index):
        raise RuntimeError("WEEKEND_GOVERNANCE_FAIL")

    cov = core.coverage_by_year(series, years_sorted)
    for year, row in cov.items():
        if int(row["selected"]) > int(row["calendar_weekdays"]):
            raise RuntimeError(f"SELECTED_EXCEEDS_CALENDAR_WEEKDAYS:{year}:{row}")
    ...
    evidence = {
        ...
        "weekday_filter": "MONDAY_FRIDAY_ONLY",
        "rejected_weekend_bars": rejected_weekend_bars,
        "coverage": cov,

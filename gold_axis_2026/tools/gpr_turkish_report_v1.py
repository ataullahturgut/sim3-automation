...
def run(root):
 ...
 'Eğitim amacı ortak negatif log marjinal olabilirliktir. Düzenlenmiş sürümlerde dönüştürülmüş parametreler üzerinde önceden sabitlenmiş Gaussian önsel cezası eklendi. Doğrulama kaybı standartlaştırılmış ölçekte %70 altın MAE ve %30 dört çıktı MAE bileşimidir. Üç tekrar arasındaki seçim yalnızca geçmiş iç doğrulamayla yapılır. Fiyat tahmini önceki gerçek fiyat × exp(tahmini log getiri) biçimindedir; bu, lognormal dağılımın medyanıdır.','',
 ...
 'LMC genişletmesi B₁⊗K_RBF + B₂⊗K_M32 + D⊗I kullanır; 40 parametre ve tam yoğun Cholesky çözümü vardır. Bu, tek ortak çekirdeğin optimizerını değiştirmekten yapısal olarak farklıdır. Üç deterministik L-BFGS-B başlangıcı, maxiter60/maxfun1600 ve λ=1 önsel kullanıldı. Bütçe sınırında kalan sonlu çözümler kaydedilir; tümünün yakınsadığı veya küresel optimum olduğu ileri sürülmez.','',

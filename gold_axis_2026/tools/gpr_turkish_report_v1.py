...
def run(root):
 ...
 'Sonuçlar, tahmin anında erişilebilir veri kurallarıyla yeniden yürütülen geçmişe dönük deneylerdir. 2025–2026 raporlamasında hiperparametreler, çekirdek ve ölçekleyici 2024-12 itibarıyla donduruldu. Sonraki aylarda yalnızca artık gözlenmiş geçmiş değerler posteriora eklenir; yeni hiperparametre, optimizer, havuz veya ensemble ağırlığı seçilmez.','',
 ...
 '| 3A | Adaptive PSO, Adaptive TLBO, TLBO-tuned PSO, DE-tuned PSO, Adaptive CROW ve PSO–TLBO. Öğrenilen dış optimizer parametreleri yalnızca tarihsel iç doğrulamayla seçildi; Adaptive PSO zamanlaması sabittir. |',
 ...
 'Stage2 rolleri karşılaştırmayı ve koşullu hibrit kabulünü belirler. Geliştirmeler ortak GP başlangıç kuralını korur; lider modelin hiperparametrelerinin aktarılmış olduğu iddia edilmez.','',
 '### Geliştirme parametrelerinin sınırları','',
 '| Yöntem | İç doğrulamada aranan parametreler / sabit program |','|---|---|',
 '| Adaptive PSO | w:0.9→0.4; c1:2.5→0.5; c2:0.5→2.5; hız sınırı parametre aralığının0.18’i; sabit program. |',
 '| Adaptive TLBO | Öğretici kazancı[0.3,1.8]; öğrenici kazancı[0.2,1.8]; öğretme faktörünün2 olma olasılığı[0,1]; azalma biçimi[0.5,3]. |',
 '| TLBO/DE-tuned PSO | w[0.2,0.95]; c1,c2[0.2,3]; hız sınırı oranı[0.05,0.8]. Dış TLBO:6×5; dış DE:8×6; iç PSO:12×20. |',
 '| Adaptive CROW | Başlangıç farkındalık olasılığı[0.02,0.8]; başlangıç uçuş uzunluğu[0.05,3]; farkındalık artışı[0,1]; uçuş azalması[0.2,3]. |',
 '| PSO–TLBO | PSO’nun aynı dört sınırı; karışım oranı[0,1]; öğretici kazancı[0,2]. |','',
 'Her tahmin tarihinde seçilen değerler, arama izleri ve kaynak kod özetleri ham sonuçta saklıdır. Son popülasyon24, nesil45 ve tekrar3; GP posteriorunun son koşullandırmasında yeniden nonlinear arama yapılmaz.','',

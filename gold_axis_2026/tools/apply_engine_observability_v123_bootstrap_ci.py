...
def patch_mobile_workflow() -> None:
    ...
    replace_once(path, "name: Gold Control Mobile UI V1.22 Multi-Expert Final\n", "name: Gold Control Mobile UI V1.23 Engine Observability Final\n")
    replace_once(path, "      - name: Verify manifest v1.22 and UI governance\n", "      - name: Verify manifest v1.23 and engine observability governance\n")
    replace_once(
        path,
    ...
    replace_once(path, "      - name: Audit local imports and manifest v1.22 screen markers\n", "      - name: Audit local imports and manifest v1.23 screen markers\n")
    replace_once(
        path,
...
def patch_app_smoke_workflow() -> None:
    path = ROOT / ".github" / "workflows" / "gold-control-app-smoke.yml"
    replace_once(
        path,
        "            gold_axis_2026/apps/decision_source.py \\\n            gold_axis_2026/apps/forecast_source.py \\\n",
        "            gold_axis_2026/apps/decision_source.py \\\n            gold_axis_2026/apps/engine_observability_contract.py \\\n            gold_axis_2026/apps/forecast_source.py \\\n",

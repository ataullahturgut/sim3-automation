...
def fa_phase(X,Y,seed,generations,center,Xv=None,Yv=None,refit=False):
    rng=np.random.default_rng(seed)
    x=common.init_population(rng,center,refit=refit)
    fit=common.population_fit(x,X,Y)
    beta0=1.0; gamma=1.0/PARAM_DIM; alpha=0.25
    ...
    for _ in range(generations):
        ...
        for i in range(1,POP_SIZE):
            for j in range(i):
                if fit[j] < fit[i]:
                    r2=np.mean(((x[i]-x[j])/(SPAN+1e-12))**2)
                    beta=beta0*np.exp(-gamma*r2)
                    noise=(common.REFIT_SIGMA if refit else 0.08*SPAN)
                    cand=x[i]+beta*(x[j]-x[i])+alpha*rng.normal(0,1,PARAM_DIM)*noise
...
def mfo_phase(X,Y,seed,generations,center,Xv=None,Yv=None,refit=False):
    rng=np.random.default_rng(seed)
    moth=common.init_population(rng,center,refit=refit)
    fit=common.population_fit(moth,X,Y)
    order=np.argsort(fit); flames,ffit=moth[order].copy(),fit[order].copy()
    b=1.0; vbest,vfit=None,math.inf
    if Xv is not None:
        vbest,vfit=common.validation_pick(moth,fit,X,Y,Xv,Yv,vbest,vfit)
    for t in range(generations):
        flame_no=max(1,int(round(POP_SIZE-t*(POP_SIZE-1)/max(1,generations-1))))
        new=np.empty_like(moth)
        for i in range(POP_SIZE):
            flame=flames[min(i,flame_no-1)]
        ...
        if Xv is not None:
            vbest,vfit=common.validation_pick(moth,fit,X,Y,Xv,Yv,vbest,vfit)
    return (vbest if Xv is not None else flames[0].copy()),(vfit if Xv is not None else float(ffit[0]))


def fpa_phase(X,Y,seed,generations,center,Xv=None,Yv=None,refit=False):
    rng=np.random.default_rng(seed)
    x=common.init_population(rng,center,refit=refit)
    fit=common.population_fit(x,X,Y)
    p=0.8; vbest,vfit=None,math.inf
    if Xv is not None:
        vbest,vfit=common.validation_pick(x,fit,X,Y,Xv,Yv,vbest,vfit)
    for _ in range(generations):
...
def fa_fpa_phase(X,Y,seed,generations,center,Xv=None,Yv=None,refit=False):
    rng=np.random.default_rng(seed)
    x=common.init_population(rng,center,refit=refit)
    fit=common.population_fit(x,X,Y)
    beta0=1.0; gamma=1.0/PARAM_DIM; alpha=0.20
    vbest,vfit=None,math.inf
    if Xv is not None:
        vbest,vfit=common.validation_pick(x,fit,X,Y,Xv,Yv,vbest,vfit)
    for _ in range(generations):
        order=np.argsort(fit); best=x[order[0]].copy()
        # Firefly intensification half-step (parity with historical ELM FA-FPA)
        for i in range(1,POP_SIZE):
            ...
            if cf<fit[i]:
                x[i],fit[i]=cand,cf
        # FPA diversification half-step
        best=x[int(np.argmin(fit))].copy()
        for i in range(POP_SIZE):
            if rng.random()<0.75:
                cand=x[i]+0.01*levy(rng,(PARAM_DIM,))*(best-x[i])
            else:
                j,k=rng.choice(POP_SIZE,2,replace=False)
                cand=x[i]+rng.random()*(x[j]-x[k])
            cand=np.clip(cand,LOWER,UPPER)
...
PHASE={"FA":fa_phase,"MFO":mfo_phase,"FPA":fpa_phase,"FA_FPA":fa_fpa_phase}
SEED_BASE={"FA":138110,"MFO":148110,"FPA":158110,"FA_FPA":168110}
...
def run_method(method):
    ...
    params={
        ...
        "MFO":{"b":1.0,"flames":"linear_POP_SIZE_to_1"},
        "FPA":{"global_pollination_probability":0.8,"levy_scale":0.01},
        "FA_FPA":{"firefly_alpha0":0.20,"alpha_decay":0.97,"fpa_global_probability":0.75,"levy_scale":0.01}
    }[method]
    out={
        "batch_id":"VW_MIDAS_ELMFIS_META_BATCH_4_V1",
    ...
    p=Path(f"vw_midas_elmfis_meta_batch_4_v1_{method.lower()}_result.json")

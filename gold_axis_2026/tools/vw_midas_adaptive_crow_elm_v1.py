...
import adaptive_crow_core_v1 as core
...
def pred(s,t):
    ...
    sd=80711+sum(map(ord,t))
    hp,outer=core.tune_outer(X,Y,sd)
    th,inner,h,a,p=core.run_crow(X,Y,hp,sd+777777)
...
def ev(B,C,a,z):
    ...
    for t in base.month_range(a,z):
        ...
        R.append({"target":t,"origin":pm,"train_rows":n,
                  ...
                  "selected_crow":{"ap0":p[0],"flight0":p[1],"ap_slope":p[2],"flight_decay":p[3]},
...
def main():
    ...
    O={"model_id":"VW_MIDAS_ADAPTIVE_CROW_ELM_V1",
       ...
       "method":{"optimizer":"Adaptive Crow Search",
                 "tuned":["ap0","flight0","ap_slope","flight_decay","hidden","alpha"],
    ...
    Path("vw_midas_adaptive_crow_elm_v1_result.json").write_text(json.dumps(O,indent=2)+"\n")

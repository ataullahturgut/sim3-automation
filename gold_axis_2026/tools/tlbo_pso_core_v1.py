...
def tlbo(X,Y,seed):
    r=np.random.default_rng(seed); lo=np.array([.2,.2,.2,.05,0,0]); hi=np.array([.95,3,3,.8,2,2])
    P=r.uniform(lo,hi,(TP,6)); F=np.array([score(q,X,Y,seed+1000+i) for i,q in enumerate(P)])
    for z in range(TI):
        teacher=P[np.argmin(F)].copy(); mean=P.mean(0); tf=int(r.integers(1,3))
        for i in range(TP):
            q=np.clip(P[i]+r.random(6)*(teacher-tf*mean),lo,hi); f=score(q,X,Y,seed+10000+z*100+i)
            if f<F[i]: P[i]=q; F[i]=f
        for i in range(TP):
            j=i
            while j==i: j=int(r.integers(TP))
            d=P[i]-P[j] if F[i]<F[j] else P[j]-P[i]
            q=np.clip(P[i]+r.random(6)*d,lo,hi); f=score(q,X,Y,seed+20000+z*100+i)
            if f<F[i]: P[i]=q; F[i]=f

...
def pred(s,t):
    k,X,Y,q,ym,ys=arr(s,t);sd=60711+sum(map(ord,t))
    hp,outer=core.de_tune(X,Y,sd)
    th,inner,h,a,p=core.pso(X,Y,hp,sd+777777)
    W,b=core.dec(th,h);B=core.beta(X,Y,W,b,a);y=(core.sig(q@W+b)@B)[0]*ys+ym
...
def main():
    ...
    O={"model_id":"VW_MIDAS_DE_TUNED_PSO_ELM_V1",
       ...
       "method":{"outer_optimizer":"DE","tuned":["w","c1","c2","vmax_frac","hidden","alpha"],
                 "hidden_grid":core.HG,"alpha_grid":core.AG,"pso_population":core.PP,"pso_iterations":core.PI,
                 "de_population":core.DP,"de_iterations":core.DI},
    ...
    Path("vw_midas_de_tuned_pso_elm_v1_result.json").write_text(json.dumps(O,indent=2)+"\n")

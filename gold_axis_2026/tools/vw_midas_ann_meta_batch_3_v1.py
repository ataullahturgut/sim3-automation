...
def woa_phase(X, Y, seed, generations, center=None, Xv=None, Yv=None):
    rng = np.random.default_rng(seed)
    whales = common.init_population(rng, center)
    fit = np.array([common.weighted_mae(w, X, Y) for w in whales])
    bi = int(np.argmin(fit))
    best, best_fit = whales[bi].copy(), float(fit[bi])
    vbest, vfit = None, math.inf
    if Xv is not None:
        vbest, vfit = common.validation_pick(whales, fit, Xv, Yv, vbest, vfit)
...
def hho_phase(X, Y, seed, generations, center=None, Xv=None, Yv=None):
    rng = np.random.default_rng(seed)
    hawks = common.init_population(rng, center)
    fit = np.array([common.weighted_mae(h, X, Y) for h in hawks])
    bi = int(np.argmin(fit))
    rabbit, rabbit_fit = hawks[bi].copy(), float(fit[bi])
    vbest, vfit = None, math.inf
    if Xv is not None:
        vbest, vfit = common.validation_pick(hawks, fit, Xv, Yv, vbest, vfit)
            ...
            if abs(escaping) >= 1:
                if q < 0.5:
                    ...
                    xn = (rabbit - mean_h) - rng.random(PARAM_DIM) * (
                        LOWER + rng.random(PARAM_DIM) * (UPPER - LOWER)
                    )
            else:
                ...
                if r >= 0.5 and abs(escaping) >= 0.5:
                    ...
                    zpos = ypos + rng.random(PARAM_DIM) * levy(rng, (PARAM_DIM,))
                    ypos, zpos = np.clip(ypos, LOWER, UPPER), np.clip(zpos, LOWER, UPPER)
                    fy = common.weighted_mae(ypos, X, Y)
                    fz = common.weighted_mae(zpos, X, Y)
                    fi = fit[i]
...
def aco_phase(X, Y, seed, generations, center=None, Xv=None, Yv=None):
    rng = np.random.default_rng(seed)
    archive = common.init_population(rng, center)
    fit = np.array([common.weighted_mae(x, X, Y) for x in archive])
    elite_n, q, xi = 8, 0.35, 0.85
    vbest, vfit = None, math.inf
    if Xv is not None:
        vbest, vfit = common.validation_pick(archive, fit, Xv, Yv, vbest, vfit)
    ...
    for _ in range(generations):
        ...
        cfit = np.array([common.weighted_mae(x, X, Y) for x in cand])
...
def bat_phase(X, Y, seed, generations, center=None, Xv=None, Yv=None):
    rng = np.random.default_rng(seed)
    ...
    for _ in range(generations):
        for i in range(len(x)):
            ...
            if rng.random() > pulse[i]:
                cand = best + 0.05 * rng.normal(0, 1, PARAM_DIM)
...
SEED_BASE = {"WOA": 97110, "HHO": 107110, "ACO": 117110, "BAT": 127110}
...
def main():
    ...
    out = {
        "batch_id": "VW_MIDAS_ANN_META_BATCH_3_V1",
        "canonical_ann": {
        ...
        "method_parameters": {
            ...
            "HHO": {"population": POP_SIZE, "energy_schedule": "2_to_0", "rapid_dives": "Levy"},
    ...
    Path("vw_midas_ann_meta_batch_3_v1_result.json").write_text(

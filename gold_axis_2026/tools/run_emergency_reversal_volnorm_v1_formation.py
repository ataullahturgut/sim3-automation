...
import emergency_reversal_volnorm_successor_v1 as m
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="/tmp/emergency-reversal-volnorm-v1-formation")
    args = parser.parse_args()
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)

    closes, evidence = m.fetch_twelve_ny17_hourly_closes(FORMATION_YEARS)
    ...
    h1 = m.timeline_hash(first)
    h2 = m.timeline_hash(second)
    if h1 != h2:
    ...
    summary = {
        ...
        "config_sha256": m.stable_sha(m.Config().as_dict()),

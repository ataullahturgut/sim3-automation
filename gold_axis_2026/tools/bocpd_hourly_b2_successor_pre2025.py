...
IDENTITY_DURATION = "BOCPD_HOURLY_B2_DURATION_RESIDUAL_V2_RESEARCH"
IDENTITY_ROBUST = "BOCPD_HOURLY_B2_ROBUST_CLIPPED_V3_RESEARCH"
IDENTITY_COMBINED = "BOCPD_HOURLY_B2_DURATION_ROBUST_V4_RESEARCH"

THRESHOLDS_RESET = [0.70, 0.80, 0.90, 0.95, 0.98]
THRESHOLDS_RISK = [0.30, 0.50, 0.70, 0.80, 0.90]
HAZARD_DAYS = [20, 40, 60, 120]
HAZARD_OBS = [d * 22 for d in HAZARD_DAYS]
WEIBULL_SHAPES = [0.70, 1.00, 1.30, 1.70, 2.00]
CLIP_LEVELS = [2.5, 3.0, 4.0, 6.0]
COOLDOWN_HOURS = 24.0
WARNING_HOURS = 120.0
RESIDUAL_HORIZON_OBS = 110
BETA = 0.5
...
def build_events(daily: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    return e[["local_date", "event_available_at_utc", "previous_daily_available_at_utc", "z", "log_return_pct"]].reset_index(drop=True)
...
def weibull_survival(r: np.ndarray | float, scale: float, shape: float) -> np.ndarray:
    a = np.asarray(r, dtype=float)
    return np.exp(-np.power(np.maximum(a, 0.0) / float(scale), float(shape)))


def hazard_vector(n: int, mode: str, expected_obs: int, shape: float = 1.0) -> np.ndarray:
    r = np.arange(n, dtype=float)
    if mode == "constant":
        return np.full(n, 1.0 / float(expected_obs), dtype=float)
    s0 = weibull_survival(r, expected_obs, shape)
    s1 = weibull_survival(r + 1.0, expected_obs, shape)
    h = 1.0 - np.divide(s1, np.maximum(s0, 1e-300))
    return np.clip(h, 1e-8, 0.95)
...
def residual_probability(run_posterior: np.ndarray, expected_obs: int, shape: float, horizon_obs: int) -> float:
    r = np.arange(len(run_posterior), dtype=float)
    s_now = weibull_survival(r, expected_obs, shape)
    s_future = weibull_survival(r + float(horizon_obs), expected_obs, shape)
    p_within = 1.0 - np.divide(s_future, np.maximum(s_now, 1e-300))
    p_within = np.clip(p_within, 0.0, 1.0)
    return float(np.dot(run_posterior, p_within))


def run_bocpd(frame: pd.DataFrame, prior: NIGPrior, expected_obs: int, hazard_mode: str = "constant", shape: float = 1.0, clip_level: float | None = None) -> pd.DataFrame:
    ...
    for row in frame.itertuples(index=False):
        x_raw = float(row.adjusted_return)
        x = float(np.clip(x_raw, -clip_level, clip_level)) if clip_level is not None else x_raw
        ...
        h = hazard_vector(len(run_posterior), hazard_mode, expected_obs, shape)
        ...
        reset_fraction = max(0.0, (expected_uninterrupted_run - current_map) / float(max(1, expected_uninterrupted_run)))
        if hazard_mode == "weibull":
            residual120 = residual_probability(new_posterior, expected_obs, shape, RESIDUAL_HORIZON_OBS)
        else:
            residual120 = 1.0 - (1.0 - 1.0 / float(expected_obs)) ** RESIDUAL_HORIZON_OBS
        ...
        next_mu[1:], next_kappa[1:], next_alpha[1:], next_beta[1:] = gmu, gk, ga, gb
...
def episode_from_risk(timeline: pd.DataFrame, threshold: float) -> pd.DataFrame:
    q = timeline.copy()
    q["prev_risk"] = q["residual120_prob"].shift(1)
    q = q[(q["residual120_prob"] >= threshold) & ((q["prev_risk"].isna()) | (q["prev_risk"] < threshold))].copy()
    return cooldown(q)
...
def score_episodes(episodes: pd.DataFrame, events: pd.DataFrame) -> dict:
    ...
    b2 = BETA * BETA
    f = (1+b2)*precision*recall/(b2*precision+recall) if precision and recall else 0.0
    return {"episodes": int(n_ep), "matched_episodes": int(len(matched)), "false_episodes": int(n_ep-len(matched)), "events": int(n_ev), "captured_events": int(len(captured)), "precision": float(precision), "recall": float(recall), "f0_5": float(f), "lead_hours": leads}
...
def validation_frame(adjusted: pd.DataFrame) -> pd.DataFrame:
    return adjusted[(adjusted["bar_start_ny"] >= pd.Timestamp("2023-11-01", tz=NY_TZ)) & (adjusted["bar_start_ny"] < pd.Timestamp("2025-01-01", tz=NY_TZ))].copy()
...
def evaluate_baseline(adjusted, prior, dev_events, valid_events):
    ...
    for days, obs in zip(HAZARD_DAYS, HAZARD_OBS):
        tl = run_bocpd(dev, prior, obs, "constant")
        for thr in THRESHOLDS_RESET:
            met = score_episodes(episode_from_reset(tl, thr), dev_events)
            grid.append({"hazard_days": days, "hazard_obs": obs, "threshold": thr, **met})
    best = max(grid, key=lambda r: score_key(r, (r["threshold"], r["hazard_days"])))
    tl = run_bocpd(validation_frame(adjusted), prior, int(best["hazard_obs"]), "constant")
    val_tl = tl[pd.to_datetime(tl["bar_start_ny"]).dt.year == VALID_YEAR].copy()
    ep = episode_from_reset(val_tl, float(best["threshold"]))
    return best, score_episodes(ep, valid_events), pd.DataFrame(grid), ep, val_tl


def evaluate_duration(adjusted, prior, dev_events, valid_events, clip_level=None):
    dev = adjusted[adjusted["year"] == DEV_YEAR].copy()
    grid = []
    for days, obs in zip(HAZARD_DAYS, HAZARD_OBS):
        for shape in WEIBULL_SHAPES:
            tl = run_bocpd(dev, prior, obs, "weibull", shape, clip_level)
            for thr in THRESHOLDS_RISK:
                met = score_episodes(episode_from_risk(tl, thr), dev_events)
                grid.append({"hazard_days": days, "hazard_obs": obs, "shape": shape, "risk_threshold": thr, "clip_level": clip_level, **met})
    best = max(grid, key=lambda r: score_key(r, (r["risk_threshold"], -abs(r["shape"]-1.0), r["hazard_days"])))
    tl = run_bocpd(validation_frame(adjusted), prior, int(best["hazard_obs"]), "weibull", float(best["shape"]), clip_level)
    val_tl = tl[pd.to_datetime(tl["bar_start_ny"]).dt.year == VALID_YEAR].copy()
    ep = episode_from_risk(val_tl, float(best["risk_threshold"]))
    return best, score_episodes(ep, valid_events), pd.DataFrame(grid), ep, val_tl


def evaluate_robust(adjusted, prior, dev_events, valid_events):
    dev = adjusted[adjusted["year"] == DEV_YEAR].copy()
    grid = []
    for clip in CLIP_LEVELS:
        for days, obs in zip(HAZARD_DAYS, HAZARD_OBS):
            tl = run_bocpd(dev, prior, obs, "constant", 1.0, clip)
            for thr in THRESHOLDS_RESET:
                met = score_episodes(episode_from_reset(tl, thr), dev_events)
                grid.append({"clip_level": clip, "hazard_days": days, "hazard_obs": obs, "threshold": thr, **met})
    best = max(grid, key=lambda r: score_key(r, (r["threshold"], r["clip_level"], r["hazard_days"])))
    tl = run_bocpd(validation_frame(adjusted), prior, int(best["hazard_obs"]), "constant", 1.0, float(best["clip_level"]))
    val_tl = tl[pd.to_datetime(tl["bar_start_ny"]).dt.year == VALID_YEAR].copy()
    ep = episode_from_reset(val_tl, float(best["threshold"]))
    return best, score_episodes(ep, valid_events), pd.DataFrame(grid), ep, val_tl


def evaluate_combined(adjusted, prior, dev_events, valid_events, duration_best):
    dev = adjusted[adjusted["year"] == DEV_YEAR].copy()
    grid = []
    for clip in CLIP_LEVELS:
        tl = run_bocpd(dev, prior, int(duration_best["hazard_obs"]), "weibull", float(duration_best["shape"]), clip)
        for thr in THRESHOLDS_RISK:
            met = score_episodes(episode_from_risk(tl, thr), dev_events)
            grid.append({"clip_level": clip, "hazard_days": int(duration_best["hazard_days"]), "hazard_obs": int(duration_best["hazard_obs"]), "shape": float(duration_best["shape"]), "risk_threshold": thr, **met})
    best = max(grid, key=lambda r: score_key(r, (r["risk_threshold"], r["clip_level"])))
    tl = run_bocpd(validation_frame(adjusted), prior, int(best["hazard_obs"]), "weibull", float(best["shape"]), float(best["clip_level"]))
    val_tl = tl[pd.to_datetime(tl["bar_start_ny"]).dt.year == VALID_YEAR].copy()
    ep = episode_from_risk(val_tl, float(best["risk_threshold"]))
    return best, score_episodes(ep, valid_events), pd.DataFrame(grid), ep, val_tl
...
def main():
    out = Path(os.environ.get("OUTPUT_DIR", "b2_successor_output"))
    out.mkdir(parents=True, exist_ok=True)
    prices = load_hourly()
    counts = prices.groupby("year").size().to_dict()
    returns = build_returns(prices)
    stats = fit_hour_adjustment(returns)
    adjusted = apply_hour_adjustment(returns, stats)
    prior = fit_prior(adjusted)
    elig = adjusted.groupby("year").size().to_dict()
    daily = load_daily()
    dev_events = build_events(daily, DEV_YEAR)
    valid_events = build_events(daily, VALID_YEAR)
    print("HOURLY_ROWS=" + json.dumps({str(k): int(v) for k, v in counts.items()}, sort_keys=True))
    print("ELIGIBLE_EXACT_1H=" + json.dumps({str(k): int(v) for k, v in elig.items()}, sort_keys=True))
    print(f"EVENT_COUNTS dev_2023={len(dev_events)} valid_2024={len(valid_events)}")

    b_best, b_val, b_grid, b_ep, b_tl = evaluate_baseline(adjusted, prior, dev_events, valid_events)
    d_best, d_val, d_grid, d_ep, d_tl = evaluate_duration(adjusted, prior, dev_events, valid_events)
    r_best, r_val, r_grid, r_ep, r_tl = evaluate_robust(adjusted, prior, dev_events, valid_events)
    c_best, c_val, c_grid, c_ep, c_tl = evaluate_combined(adjusted, prior, dev_events, valid_events, d_best)

    models = {
        IDENTITY_BASE: {"dev_selected": b_best, "validation_2024": b_val},
        IDENTITY_DURATION: {"dev_selected": d_best, "validation_2024": d_val},
        IDENTITY_ROBUST: {"dev_selected": r_best, "validation_2024": r_val},
        IDENTITY_COMBINED: {"dev_selected": c_best, "validation_2024": c_val},
    }
    ranked = sorted([{"identity": k, **v["validation_2024"]} for k, v in models.items()], key=lambda x: (x["f0_5"], x["precision"], x["recall"], -x["episodes"]), reverse=True)
    summary = {
        ...
        "2025_used": False,
        "hourly_rows": {str(k): int(v) for k, v in counts.items()},
        "eligible_exact_1h_returns": {str(k): int(v) for k, v in elig.items()},
        "development_event_count": int(len(dev_events)),
        "validation_event_count": int(len(valid_events)),
        "models": models,
        "validation_ranking": ranked,
        "promotion": "NONE_AUTOMATIC",
    }

    for name, grid in [("baseline_grid.csv", b_grid), ("duration_grid.csv", d_grid), ("robust_grid.csv", r_grid), ("combined_grid.csv", c_grid)]:
        grid.to_csv(out / name, index=False)
    dev_events.to_csv(out / "events_2023.csv", index=False)
    valid_events.to_csv(out / "events_2024.csv", index=False)
    for prefix, ep, tl in [("baseline", b_ep, b_tl), ("duration", d_ep, d_tl), ("robust", r_ep, r_tl), ("combined", c_ep, c_tl)]:
        ep.to_csv(out / f"{prefix}_2024_episode_onsets.csv", index=False)
        tl.to_csv(out / f"{prefix}_2024_full_timeline.csv", index=False)
    stats.to_csv(out / "hour_adjustment_2022.csv", index=False)
    (out / "summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True, default=str), encoding="utf-8")
    print("MODEL_COMPARISON_SUMMARY=" + json.dumps(summary, sort_keys=True, default=str))

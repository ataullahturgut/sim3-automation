...
import pso_tlbo_hybrid_core_v1 as core
...
def pred(s,t):
    k,X,Y,q,ym,ys=arr(s,t)
    sd=90711+sum(map(ord,t))
    hp,outer=core.tune_outer(X,Y,sd)
    th,inner,h,a,p=core.run_hybrid(X,Y,hp,sd+777777)
...
def ev(B,C,a,z):
    ...
    for t in base.month_range(a,z):
        ...
        R.append({"target":t,"origin":pm,"train_rows":n,
                  "outer_inner_score":o,"final_inner_fitness":i,
                  "selected_hidden":h,"selected_alpha":al,
                  "selected_hybrid":{"w":p[0],"c1":p[1],"c2":p[2],"vmax_frac":p[3],"mix":p[4],"tlbo_gain":p[5]},
                  "forecast":float(B.core_gold[pm]*math.exp(float(y[0]))),
                  "actual":float(B.core_gold[t]),"rw":float(B.core_gold[pm])})
    return R

def main():
    d=os.environ["NEON_DATABASE_URL"]; B=base.load_data(d)
    ...
    O={"model_id":"VW_MIDAS_PSO_TLBO_HYBRID_ELM_V1",
       ...
       "method":{"optimizer":"PSO-TLBO Hybrid",
                 "tuned":["w","c1","c2","vmax_frac","mix","tlbo_gain","hidden","alpha"],
                 "hidden_grid":core.HG,"alpha_grid":core.AG,
                 "population":core.POP,"iterations":core.ITERS},
    ...
    Path("vw_midas_pso_tlbo_hybrid_elm_v1_result.json").write_text(json.dumps(O,indent=2)+"\n")

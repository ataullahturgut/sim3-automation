...
def run(root):
    ...
    lines+=['','## Kontrol ve Uyum Özeti','','- DEV-only selection: PASS.','- 2025/2026 optimizer, parent, alpha and pool selection exclusion: PASS for this GPR program under the predeclared frozen external hyperparameter protocol.','- Random split: NONE.','- Chronological model fitting and frozen-pool weight updates: PASS; target/future-label invariance tested. Full-DEV pool and alpha selection remains conditional DEV selection, not an independent test.','- DB: READ_ONLY; invariant equality checked.','- Scientific/numerical gate: recorded per model and period; no failed subset ranked.','- Full-DEV learned-weight fits: DIAGNOSTIC ONLY.','- Main roadmap/ledger: updated with stage status, run/job/artifact/commit lineage.','',

...
def main() -> None:
    ...
    parser.add_argument("--input-dir", default="/tmp/emergency-reversal-volnorm-v2-2025")
    args = parser.parse_args()
    root = Path(args.input_dir)

    payload = json.loads((root / "engine_2025_frozen_payload.json").read_text(encoding="utf-8"))
    summary_engine = json.loads((root / "engine_2025_summary.json").read_text(encoding="utf-8"))
    ...
    if summary_engine["timeline_sha256"] != payload["timeline_sha256"]:
    ...
    if len(event_map) != 19:
        raise RuntimeError("FROZEN_EVENT_COUNT_FAIL")
    ...
    # Secondary descriptive proximity on the engine's observed-date clock only.
    ...
    for date, direction, tier, z in EVENTS:
        if date not in pos:
            proximity.append({"date": date, "direction": direction, "nearest_aligned_distance": None, "nearest_opposed_distance": None})
            continue
        p = pos[date]
    ...
    summary = {
        ...
        "alerts_no_same_day_volatility_overlay": sum(x["result"] == "NO_SAME_DAY_VOLATILITY_OVERLAY" for x in alert_overlay),
        "aligned_within_1_observed_day": sum(x["nearest_aligned_distance"] is not None and x["nearest_aligned_distance"] <= 1 for x in proximity),
        "aligned_within_3_observed_days": sum(x["nearest_aligned_distance"] is not None and x["nearest_aligned_distance"] <= 3 for x in proximity),

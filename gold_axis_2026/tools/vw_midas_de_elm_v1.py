...
def de_optimize(X,Y,seed,pop_size=24,generations=35,F=0.7,CR=0.9):
...
def predict(samples,t):
    ...
    theta,fit=de_optimize(X,Y,seed=5711+sum(map(ord,t)))
    W,b=decode(theta)
...
def ev(b,cache,a,z):
    ...
    for t in base.month_range(a,z):
        ...
        rows.append({"target":t,"origin":pm,"train_rows":n,"de_inner_fitness":fit,
...
def main():
    ...
    out={"model_id":"VW_MIDAS_DE_ELM_V1",
         "method":{"elm_hidden":HIDDEN,"activation":"sigmoid","ridge_alpha":ALPHA,
                   "de_scope":"hidden_input_weights_and_biases","population":24,"generations":35,
                   "F":0.7,"CR":0.9,
    ...
    Path("vw_midas_de_elm_v1_result.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\n")

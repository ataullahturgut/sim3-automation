...
SERIES="TWELVE_DATA_XAUUSD_DAILY_R2"
...
def load_series(end_date):
    ...
    params={
        "symbol":"XAU/USD","interval":"1day",
        "start_date":"2010-01-01","end_date":end_date.isoformat(),
        "order":"ASC","outputsize":5000,"format":"JSON",
        "timezone":"America/New_York"
    }
    r=requests.get("https://api.twelvedata.com/time_series",params=params,
                   headers={"Authorization":f"apikey {key}","User-Agent":"GoldControl-ACDPOT-R2/1.0"},
                   timeout=(10,120))
    payload=r.json()
    if isinstance(payload,dict) and payload.get("status")=="error":
        raise RuntimeError(f"TWELVE_ERROR:{payload.get('code')}:{payload.get('message')}")
    r.raise_for_status()
    vals=payload.get("values") if isinstance(payload,dict) else None
    if not isinstance(vals,list) or not vals: raise RuntimeError("TWELVE_NO_VALUES")
    by={}
    for row in vals:
...
def main():
    ...
    if a.stage=="pre2025":
        ...
        passed=gate24(m24)
        cfg={
            "identity":IDENTITY,"series":SERIES,"provider":"Twelve Data","symbol":"XAU/USD","interval":"1day","formation_end":FORM_END.isoformat(),

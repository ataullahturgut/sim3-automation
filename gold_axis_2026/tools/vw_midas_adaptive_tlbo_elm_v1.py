...
import adaptive_tlbo_core_v1 as core
...
def pred(s,t):
    ...
    sd=70711+sum(map(ord,t))
    hp,outer=core.tune_outer(X,Y,sd)
    th,inner,h,a,p=core.run_itlbo(X,Y,hp,sd+777777)
...
def ev(B,C,a,z):
    ...
    for t in base.month_range(a,z):
        ...
        R.append({"target":t,"origin":pm,"train_rows":n,
                  ...
                  "selected_itlbo":{"teach_gain":p[0],"learn_gain":p[1],"tf2_probability":p[2],"decay":p[3]},
...
def main():
    ...
    O={"model_id":"VW_MIDAS_ADAPTIVE_TLBO_ELM_V1",
       ...
       "method":{"optimizer":"Adaptive/Improved TLBO",
                 "tuned":["teach_gain","learn_gain","tf2_probability","decay","hidden","alpha"],
    ...
    Path("vw_midas_adaptive_tlbo_elm_v1_result.json").write_text(json.dumps(O,indent=2)+"\n")

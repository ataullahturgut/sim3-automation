...
def _session() -> requests.Session:
    ...
    s.headers.update({"User-Agent": "GoldControl-BOCPD-Hourly-B/1.1", "Accept": "application/json"})
    return s
...
def apply_xau_session_gate(frame: pd.DataFrame, label: str) -> pd.DataFrame:
    out = frame.sort_values("bar_start_utc").drop_duplicates("bar_start_utc", keep="first").copy()
    ...
    excluded = int(len(out) - len(kept))
    print(f"SESSION_GATE label={label} raw_rows={len(out)} kept_rows={len(kept)} excluded_rows={excluded}")
    if kept.empty:
        raise RuntimeError(f"SESSION_GATE_EMPTY:{label}")
    return kept
...
def load_formation_prices() -> pd.DataFrame:
    ...
    return apply_xau_session_gate(frame, "formation_2023_2024")
...
def fetch_2025_prices() -> pd.DataFrame:
    ...
    for idx, start in enumerate(starts, start=1):
        ...
        for row in values:
            ...
            if pd.Timestamp(start) <= ts < pd.Timestamp(end):
                ...
                valid += 1
        print(f"CHALLENGE_FETCH_CHUNK={idx} raw_valid_rows={valid} start={start.date()} end_exclusive={end.date()}")
        time.sleep(2)
    ...
    raw_rows = len(frame)
    frame = apply_xau_session_gate(frame, "challenge_2025")
    if not (5700 <= len(frame) <= 6100):
        raise RuntimeError(f"CHALLENGE_SESSION_GATED_ROW_COUNT_IMPLAUSIBLE:raw={raw_rows}:kept={len(frame)}")
    print(f"CHALLENGE_FETCH_COMPLETE raw_rows={raw_rows} session_rows={len(frame)} raw_market_values_logged=NO")
    return frame
...
def build_returns(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    actual_hours_formation = sorted(frame[frame["bar_start_ny"].dt.year.isin([2023, 2024])]["bar_start_hour"].unique().tolist())
    actual_hours_challenge = sorted(frame[frame["bar_start_ny"].dt.year == 2025]["bar_start_hour"].unique().tolist())
    if actual_hours_formation != EXPECTED_HOURS:
        raise RuntimeError(f"FORMATION_ELIGIBLE_HOUR_SET_MISMATCH:{actual_hours_formation}")
    if actual_hours_challenge != EXPECTED_HOURS:
        raise RuntimeError(f"CHALLENGE_ELIGIBLE_HOUR_SET_MISMATCH:{actual_hours_challenge}")
...
def main() -> None:
    ...
    monthly = resets["bar_start_ny"].dt.strftime("%Y-%m").value_counts().sort_index().to_dict()
    ...
    summary = {
        "identity": IDENTITY,
        "formation_source_rows_2023_2024_after_session_gate": int(len(formation_prices)),
        "challenge_source_rows_2025_after_session_gate": int(len(challenge_prices)),
        ...
        "corrective_history": "initial 2025 raw provider row-count check failed before model scoring; R1 applies the pre-score frozen XAU session gate only",
    }
    ...
    print("CANDIDATE_B_ALL_RESETS_BEGIN")
    for row in resets.itertuples(index=False):
        print(f"RESET bar_start_ny={row.bar_start_ny.isoformat()} available_at_utc={row.available_at_utc.isoformat()} reset_fraction={row.reset_fraction:.6f} map_run={row.map_run}")
    print("CANDIDATE_B_ALL_RESETS_END")

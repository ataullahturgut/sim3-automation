...
def read_csv(url):
    ...
    raise err
# Archived origin-safe monthly/model/Tactical evidence
A=pd.read_csv(OUT/'autopsy_2023_direction.csv')
A['month']=A['month'].astype(str)
# Daily gold for intramonth shock layer
...
# Direction history for frozen pre-2023 emergency training
...
# Downside rule is the already-frozen Emergency R1 rule
down_rule=(-.03,-.04,50)
# Upside shock rule: choose only from 2010-2022; no 2023 labels used.
...
def eval_up(rule,a,b):
    x=M[(M.target>=a)&(M.target<=b)&(M.dir3==-1)]
    corr=dmg=tr=0
...
for _,r in A.iterrows():
    ...
    final_dir=(-prior if eh else prior) if prior!=0 else prior
    final_hit=int(final_dir==actual) if actual!=0 else 0
    action='KEEP_PRIOR'
    if disagree: action='LOW_CONFIDENCE_KEEP_PRIOR'
    if eh: action='INTRAMONTH_EMERGENCY_FLIP'
    rows.append(dict(month=m,origin_gold=r.origin_gold,actual_gold=r.actual_gold,actual_move_pct=r.actual_move_pct,actual_dir=actual,mom3_dir=prior,mom1_dir=int(r.mom1_dir),vw_dir=int(r.vw_dir),patch_dir=int(r.patch_dir),idma_dir=int(r.idma_dir),tactical_relation=r.tactical_relation,disagreement='+'.join(disagree) if disagree else '',start_dir_hit=start_hit,emergency_type=etype,emergency_date='' if pd.isna(ed) else ed.date().isoformat(),intramonth_final_dir=final_dir,intramonthish_hit=final_hit,action=action,vw_ape=abs(r.vw_forecast-r.actual_gold)/r.actual_gold*100,patch_ape=abs(r.patch_forecast-r.actual_gold)/r.actual_gold*100,idma_ape=abs(r.idma_forecast-r.actual_gold)/r.actual_gold*100,mom3_ape=abs(r.mom3_forecast-r.actual_gold)/r.actual_gold*100))
R=pd.DataFrame(rows); R.to_csv(OUT/'FULL_SYSTEM_2023_MONTHLY_REPLAY.csv',index=False)
nonflat=R[R.actual_dir!=0]
strict=(R.start_dir_hit.mean()); econ=nonflat.start_dir_hit.mean(); final=nonflat.intramonthish_hit.mean()
...
for _,r in R.iterrows(): lines.append(f"|{r.month}|{r.actual_dir:+d}|{r.mom3_dir:+d}|{r.mom1_dir:+d}|{r.vw_dir:+d}|{r.tactical_relation}|{r.emergency_type or '-'}|{r.emergency_date or '-'}|{r.intramonthish_final_dir:+d}|{r.action}|")

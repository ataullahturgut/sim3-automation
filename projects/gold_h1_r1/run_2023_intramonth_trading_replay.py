...
D['fast']=fast
wd=D.copy(); wd['wk_start']=(wd.date-pd.to_timedelta((wd.date.dt.weekday+1)%7,unit='D')).dt.normalize(); W=wd.groupby('wk_start').agg(wclose=('close','last')).reset_index().sort_values('wk_start')
...
W['slow']=slow; W['known_date']=W.wk_start+pd.Timedelta(days=6); D=pd.merge_asof(D.sort_values('date'),W[['known_date','slow']].rename(columns={'known_date':'date'}).sort_values('date'),on='date',direction='backward'); D['slow']=D.slow.fillna(0).astype(int)
M=pd.read_csv(OUT/'monthly_direction_min_2010_2023.csv'); M['target']=pd.to_datetime(M.target).dt.to_period('M').astype(str); prior=dict(zip(M.target,M.dir3.astype(int))); D['prior']=D.month.map(prior).fillna(0).astype(int)
def make_pos(df,policy):
  pos=[]; cur=None; opp_count=0; last_month=None
  for _,r in df.iterrows():
    p=int(r.prior); f=int(r.fast); s=int(r.slow); m=r.month; base=1 if p==1 else 0
    ...
    if policy=='monthly': cur=base
      ...
      opp=(f!=0 and s!=0 and f==s and f==-p); same=(f!=0 and s!=0 and f==s and f==p); opp_count=opp_count+1 if opp else 0; need=1 if policy=='dual' else 2
      if opp_count>=need: cur=1 if f==1 else 0
      elif same: cur=base
    pos.append(cur)
  return pd.Series(pos,index=df.index,dtype=int)
def eval_policy(policy,a,b,cost=.001):
  x=D[(D.date>=pd.Timestamp(a))&(D.date<=pd.Timestamp(b))].copy(); x['pos']=make_pos(x,policy); x['turn']=x.pos.diff().abs().fillna(0); x['strat_ret']=x.pos.shift(1).fillna(x.pos.iloc[0])*x.ret.fillna(0)-cost*x.turn; eq=(1+x.strat_ret).cumprod(); return eq.iloc[-1]-1,(eq/eq.cummax()-1).min(),int(x.turn.sum())
...
base_dev=util(S[(S.policy=='monthly')&(S.period=='DEV')].iloc[0]); base_val=util(S[(S.policy=='monthly')&(S.period=='VAL')].iloc[0]); cands=[]
for p in policies:
...
chosen=max(cands)[-1]
X=D[(D.date>='2023-01-01')&(D.date<='2023-12-31')].copy(); X['pos']=make_pos(X,chosen); X['turn']=X.pos.diff().abs().fillna(0); X['strat_ret']=X.pos.shift(1).fillna(X.pos.iloc[0])*X.ret.fillna(0)-.001*X.turn; X['equity']=(1+X.strat_ret).cumprod(); X.to_csv(OUT/'INTRAMONTH_2023_DAILY_REPLAY.csv',index=False)
mrs=[]
for m,g in X.groupby('month'):
  dates=g.loc[g.turn>0,['date','pos']]; actions='; '.join(f"{d.date().isoformat()} {'AL' if p==1 else 'SAT'}" for d,p in zip(dates.date,dates.pos)) or '-'; first_change='' if dates.empty else dates.date.iloc[0].date().isoformat(); mrs.append([m,int(g.prior.iloc[0]),int(g.pos.iloc[0]),int(g.pos.iloc[-1]),float((1+g.strat_ret).prod()-1),float(g.close.iloc[-1]/g.close.iloc[0]-1),int(g.turn.sum()),first_change,actions])
...
net=float(X.equity.iloc[-1]-1); mdd=float((X.equity/X.equity.cummax()-1).min()); trades=int(X.turn.sum()); bh=float(X.close.iloc[-1]/X.close.iloc[0]-1); bn,bd,bt=eval_policy('monthly','2023-01-01','2023-12-31')
